...
import { cardService } from "../../api/product/card.service";
      ...
      const cards = (res.data.data || []).map((card) => {
        ...
        switch (card.cardType) {
          ...
          case "Виртуальная":
            ...
            break;
          case "CoBadging":
          case "Ко-брендинговая":
            cardType = 4;
            break;
          default:
            cardType = Number(card.cardType) || 0;
        }
        ...
        switch (card.currency) {
          ...
          case "RUB":
            ...
            break;
          default:
            currency = Number(card.currency) || 0;
        }
        ...
        return {
          ...
          dailyCashWithdrawalOwnAtmLimit:
            Number(card.dailyCashWithdrawalOwnAtmLimit) || 0,
          dailyCashWithdrawalDomesticOtherAtmLimit:
            Number(card.dailyCashWithdrawalDomesticOtherAtmLimit) || 0,
          dailyCashWithdrawalAbroadAtmLimit:
            Number(card.dailyCashWithdrawalAbroadAtmLimit) || 0,
          monthlyCashWithdrawalOwnAtmLimit:
            Number(card.monthlyCashWithdrawalOwnAtmLimit) || 0,
          monthlyCashWithdrawalDomesticOtherAtmLimit:
            Number(card.monthlyCashWithdrawalDomesticOtherAtmLimit) || 0,
          monthlyCashWithdrawalAbroadAtmLimit:
            Number(card.monthlyCashWithdrawalAbroadAtmLimit) || 0,
          dailyPosPurchaseDomesticLimit:
            Number(card.dailyPosPurchaseDomesticLimit) || 0,
          dailyPosPurchaseAbroadLimit:
            Number(card.dailyPosPurchaseAbroadLimit) || 0,
          monthlyPosPurchaseDomesticLimit:
            Number(card.monthlyPosPurchaseDomesticLimit) || 0,
          monthlyPosPurchaseAbroadLimit:
            Number(card.monthlyPosPurchaseAbroadLimit) || 0,
          dailyOnlinePurchaseLimit: Number(card.dailyOnlinePurchaseLimit) || 0,
          monthlyOnlinePurchaseLimit:
            Number(card.monthlyOnlinePurchaseLimit) || 0,
          dailyTotalSpendingLimit: Number(card.dailyTotalSpendingLimit) || 0,
          monthlyTotalSpendingLimit:
            Number(card.monthlyTotalSpendingLimit) || 0,
