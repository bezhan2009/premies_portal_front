      ...
      {clientsData.length > 1 && (
        <div className="processing-integration__client-selector">
          <div className="client-selector">
            <h3 className="client-selector__title">Найдено клиентов: {clientsData.length}</h3>
        ...
        <div className="limits-table">
          <div className="limits-table__header">
            ...
            <div className="limits-table__actions">
              <button onClick={handleExportClientInfo} className="export-excel-btn" style={{ marginRight: 10 }}>Экспорт в Excel</button>
              <button onClick={copySelectedClientToClipboard} className="limits-table__action-btn limits-table__action-btn--secondary">Скопировать JSON клиента</button>
              {clientsData.length > 1 && (
                <button onClick={copyAllClientsToClipboard} className="limits-table__action-btn limits-table__action-btn--secondary">Скопировать JSON всех клиентов</button>
              )}
            </div>
          </div>

          <div className="limits-table__wrapper">
            <Table dataSource={[selectedClient]} rowKey="client_code" pagination={false} bordered scroll={{ x: "max-content" }}>
              {tableData.map((item) => (
                <Table.Column title={item.label} key={item.key} render={() => item.value || (item.value === 0 ? 0 : "Не указано")} />
              ))}
            </Table>
          </div>
          ...
          <div className="limits-table__footer">
            <div className="limits-table__stats">
              <span className="limits-table__stat">ФИО: {selectedClient.surname} {selectedClient.name} {selectedClient.patronymic}</span>
              <span className="limits-table__stat">Телефон: {selectedClient.phone}</span>
              <span className="limits-table__stat">ИНН: {selectedClient.tax_code}</span>
              <span className="limits-table__stat">Код клиента: {selectedClient.client_code}</span>
            </div>
          </div>
        </div>
      </div>
