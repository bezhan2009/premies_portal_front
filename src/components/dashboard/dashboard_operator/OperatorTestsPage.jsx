    ...
    return (
        <div className="tests-module">
            <aside className="kb-sidebar">
                <h3>Тесты</h3>
                <button className="kb-add-btn" onClick={() => openModal('test', 'create')}>+ Добавить тест</button>
                <ul>
                    {tests.map(t => (
                        <li
                            key={t.ID}
                ...
                {testDetail ? (
                    <div className="kb-detail">
                        ...
                        <div className="kb-items">
                            {testDetail.Questions?.map(q => (
                                <div key={q.ID} className={`question-card question-${q.type}`}>
                                    <h4>{q.text}</h4>
                                    <p className="question-type">Тип: {QUESTION_TYPE_LABELS[q.type]}</p>
                                    ...
                                    {['single_choice', 'multiple_choice'].includes(q.type) && (
                                        <ul className="options-list">
                                            {q.Options?.map(o => (
                                                <li
                                                    key={o.ID}
                                                    className={`option-item ${o.is_correct ? 'correct' : ''}`}
                                                >
                                                    <label>
                                                        <input
                                                    ...
                                                    <div className="option-actions">
                                                        <span onClick={() => openModal('option', 'edit', o)}>✎</span>
                                                        <span onClick={() => handleDelete('option', o.ID)}>🗑</span>
                                                    </div>
                                                </li>
                                            ))}
                                        </ul>
                                        ...
                                        <>
                                            {q.Options?.[0]?.correct_text && (
                                                <p className="correct-text">
                                                    Правильный ответ: <strong>{q.Options[0].correct_text}</strong>
                                                    <span
                                                        className="kb-action-btn"
                                                        onClick={() => openModal('option', 'edit', {
                                                            ...q.Options[0],
                                                            question_id: q.ID
                                                        })}
                                                    >✎</span>
                                                    <span
                                                        className="kb-action-btn"
                                                        onClick={() => handleDelete('option', q.Options[0].ID)}
                                                    >🗑</span>
                                                </p>
                                            )}
                                            {!q.Options?.length && (
                                    ...
                                    <div className="kb-doc-actions">
                                        <span onClick={() => openModal('question', 'edit', q)}>✎</span>
                                        <span onClick={() => handleDelete('question', q.ID)}>🗑</span>
                                    </div>
                        ...
                        <button className="kb-add-btn"
                                onClick={() => openModal('question', 'create', { test_id: testDetail.ID })}>
                            Добавить вопрос
                        </button>
                                    ...
                                    <div className="answers-grid">
                                        {answers.map((a) => {
                                            return (
                                                <div
                                                    key={a.ID}
                                                    className={`answer-card ${a.is_correct_answer ? "correct-answer" : "wrong-answer"}`}
                                                >
                                                    <div className="answer-header">
                                                        <strong>{a.user?.full_name || "Неизвестный пользователь"}</strong>
            ...
            {QUESTION_TYPE_LABELS[a.type]}
                                                    ...
                                                    {a.type === "single_choice" && (
                                                        <>
                                                            ...
                                                            <p>
                                                                ...
                                                                {a.question?.Options?.find((opt) => opt.is_correct)?.text ||
                                                                    "Нет данных"}
                                                            </p>
                                                        </>
                                                    )}
                                                    ...
                                                    {a.type === "multiple_choice" && (
                                                        <>
                                                            ...
                                                            <ul className="multi-list">
                                                                {a.question?.Options?.filter((opt) => opt.is_correct)?.map(
                                                                    (opt) => (
                                                                        <li key={opt.ID}>{opt.text}</li>
                                                                    )
                                                                )}
                                                    ...
                                                    {a.type === "text" && (
                                                        <>
                                                            ...
                                                            <p>
                                                                ...
                                                                {a.question?.Options?.[0]?.correct_text || "Нет данных"}
                                                            </p>
                                                        </>
                                                    )}
                                                </div>
...
function CrudModal({ modal, onClose, onSave }) {
    ...
    const validate = () => {
        ...
        if (modal.entity === 'test') {
            if (!form.Title || !form.Title.trim()) newErrors.Title = 'Название обязательно';
            if (!form.description || !form.description.trim()) newErrors.description = 'Описание обязательно';
        }
        if (modal.entity === 'question') {
            if (!form.text || !form.text.trim()) newErrors.text = 'Текст вопроса обязателен';
            if (!form.type || form.type.trim() === '') newErrors.type = 'Тип вопроса обязателен';
        }
        if (modal.entity === 'option') {
            if (!form.text || !form.text.trim()) newErrors.text = 'Текст варианта обязателен';
        }
        setErrors(newErrors);
    ...
    const handleSubmit = () => {
        ...
        onSave(modal.entity, modal.mode, form, modal.data?.ID || form.question_id || form.test_id);
    };

    return (
        <div className="kb-modal-overlay">
            <div className="kb-modal">
                <h3>{modal.mode === 'create' ? 'Создать' : 'Редактировать'} {modal.entity}</h3>
                <div className="kb-modal-content">
                    ...
                    {modal.entity === 'option' && (
                        <>
                            <input name="text" value={form.text || ''} onChange={handleChange} placeholder="Текст варианта" />
                            {errors.text && <span className="error-msg">{errors.text}</span>}
                            <input name="correct_text" value={form.correct_text || ''} onChange={handleChange} placeholder="Правильный текст (для text-вопросов)" />
                            <select name="is_correct" value={String(form.is_correct)} onChange={handleChange}>
