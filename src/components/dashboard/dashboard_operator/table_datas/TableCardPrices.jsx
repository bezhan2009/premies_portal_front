            ...
            <div className="add-card-form" style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                    <input
                        ...
                        onChange={(e) => setNewCard({ ...newCard, dcl_name: e.target.value })}
                    ...
                    />
                    <button onClick={handleAdd} className="action-buttons__btn">Добавить</button>
                </div>
            </div>
                    ...
                    {Array.isArray(cards) && cards.length > 0 ? (
                        cards.map((card) => (
                            <tr key={card.id}>
                                <td onDoubleClick={() => handleDoubleClick(card)}>
                                    {editId === card.id ? (
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setEditedCard({ ...editedCard, dcl_name: e.target.value })
                                            }
                                            onKeyDown={(e) => e.key === "Enter" && handleSave(card.id)}
                                            autoFocus
                                        />
                                    ) : (
                                        card.dcl_name
                                ...
                                <td onDoubleClick={() => handleDoubleClick(card)}>
                                    {editId === card.id ? (
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setEditedCard({ ...editedCard, coast_cards: Number(e.target.value) })
                                            }
                                            onKeyDown={(e) => e.key === "Enter" && handleSave(card.id)}
                                        />
                                    ) : (
                                ...
                                <td onDoubleClick={() => handleDoubleClick(card)}>
                                    {editId === card.id ? (
                                        <input
                                            ...
                                            onChange={(e) =>
                                                setEditedCard({ ...editedCard, coast_credits: Number(e.target.value) })
                                            }
                                            onKeyDown={(e) => e.key === "Enter" && handleSave(card.id)}
                                        />
                                    ) : (
                                ...
                                <td>
                                    {editId === card.id ? (
                                        <button onClick={() => handleSave(card.id)} className="action-buttons__btn">Сохранить</button>
                                    ) : (
                                        <button onClick={() => handleDelete(card.id)} className="action-buttons__btn">Удалить</button>
                                    )}
                                </td>
                            </tr>
                        ))
