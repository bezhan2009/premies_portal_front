...
export default function DashboardOperatorTransactionSearch() {
  ...
  const { totalAmount, totalCountByResponse } = React.useMemo(() => {
    ...
    transactions.forEach((tx) => {
      ...
      // Если дебетовая операция (тип 2), вычитаем из общей суммы?
      // Обычно "общая сумма" это оборот, но пользователь просил "сумма".
      // Для статистики лучше просто суммировать модули или учитывать знак.
      // Оставим простое суммирование для начала, или как в formatAmount.
      if (type === 2) {
        total -= val;
      } else {
        total += val;
