...
export default function DashboardOperatorTransactionSearch() {
    ...
    // Проверка доступа (оставлена без изменений)
    ...
    // ---------- Поля фильтрации (все параметры эндпоинта) ----------
    ...
    // ---------- Обработка ограниченного доступа (из оригинального кода) ----------
    useEffect(() => {
        if (!hasAccess) {
    ...
    const getStatusBadge = (responseCode, reversal, message) => {
        if (reversal) return <span className="status-badge status-badge--reversed">{message}</span>;
        switch (responseCode) {
    ...
    // ---------- Единый поиск через search-transactions ----------
    const handleSearch = useCallback(async () => {
        if (!validateSearch()) return;

        setIsLoading(true);
        try {
            // Собираем только заполненные параметры
            const params = {};
            ...
            if (transactionType) params.transactionType = parseInt(transactionType, 10);
        ...
        utrnno, currency, conCurrency, reversal, transactionType, atmId, mcc,
        account, fromDate, toDate, fromTime, toTime, validateSearch, showAlert
    ]);

    // Автоматический поиск при наличии id в URL (ограниченный доступ)
    useEffect(() => {
        if (id?.length) {
            setCardId(id);
            handleSearch();
                            ...
                            <div className="search-card">
                                <div className="search-card__content">
                                    ...
                                    <div className="search-card__row">
                                        ...
                                        <div className="search-card__input-group">
                                            <label htmlFor="responseCode" className="search-card__label">
                                                Response code
                                            </label>
                                            <input
                                                type="text"
                                    ...
                                    <div className="search-card__row">
                                        ...
                                        <div className="search-card__input-group">
                                            <label htmlFor="conCurrency" className="search-card__label">
                                                Валюта конверс.
                                            </label>
                                            <input
                                                type="text"
                                    ...
                                    <div className="limits-table__wrapper">
                                        <table className="limits-table__content">
                                            ...
                                            <tr>
                                                ...
                                                <th onClick={() => requestSort("localTransactionDate")} className="limits-table__th sortable-header">
                                                    Дата <SortIcon sortConfig={sortConfig} sortKey="localTransactionDate" />
                                                </th>
                                                <th onClick={() => requestSort("responseDescription")} className="limits-table__th sortable-header">
                                                    Статус <SortIcon sortConfig={sortConfig} sortKey="responseDescription" />
                                                </th>
                                                <th onClick={() => requestSort("cardNumber")} className="limits-table__th sortable-header">
                                                    Номер карты <SortIcon sortConfig={sortConfig} sortKey="cardNumber" />
                                                </th>
                                                <th onClick={() => requestSort("cardId")} className="limits-table__th sortable-header">
                                                    ID карты <SortIcon sortConfig={sortConfig} sortKey="cardId" />
                                                </th>
                                                <th onClick={() => requestSort("transactionTypeName")} className="limits-table__th sortable-header">
                                                    Тип операции <SortIcon sortConfig={sortConfig} sortKey="transactionTypeName" />
                                                </th>
                                                <th onClick={() => requestSort("amount")} className="limits-table__th sortable-header">
                                                    Сумма операции <SortIcon sortConfig={sortConfig} sortKey="amount" />
                                                </th>
                                                <th onClick={() => requestSort("currency")} className="limits-table__th sortable-header">
                                                    Валюта <SortIcon sortConfig={sortConfig} sortKey="currency" />
                                                </th>
                                                <th onClick={() => requestSort("conamt")} className="limits-table__th sortable-header">
                                                    Сумма в валюте карты <SortIcon sortConfig={sortConfig} sortKey="conamt" />
                                                </th>
                                                <th onClick={() => requestSort("acctbal")} className="limits-table__th sortable-header">
                                                    Доступный баланс <SortIcon sortConfig={sortConfig} sortKey="acctbal" />
                                                </th>
                                                <th onClick={() => requestSort("netbal")} className="limits-table__th sortable-header">
                                                    Баланс карты <SortIcon sortConfig={sortConfig} sortKey="netbal" />
                                                </th>
                                                <th onClick={() => requestSort("utrnno")} className="limits-table__th sortable-header">
                                                    Номер операции в ПЦ <SortIcon sortConfig={sortConfig} sortKey="utrnno" />
                                                </th>
                                                <th onClick={() => requestSort("terminalId")} className="limits-table__th sortable-header">
                                                    ID терминала <SortIcon sortConfig={sortConfig} sortKey="terminalId" />
                                                </th>
                                                <th onClick={() => requestSort("atmId")} className="limits-table__th sortable-header">
                                                    ID АТМ <SortIcon sortConfig={sortConfig} sortKey="atmId" />
                                                </th>
                                                <th onClick={() => requestSort("reqamt")} className="limits-table__th sortable-header">
                                                    Запрошенная сумма <SortIcon sortConfig={sortConfig} sortKey="reqamt" />
                                                </th>
                                                <th onClick={() => requestSort("terminalAddress")} className="limits-table__th sortable-header">
                                                    Адрес терминала <SortIcon sortConfig={sortConfig} sortKey="terminalAddress" />
                                                </th>
                                                <th onClick={() => requestSort("mcc")} className="limits-table__th sortable-header">
                                                    MCC код <SortIcon sortConfig={sortConfig} sortKey="mcc" />
                                                </th>
                                                <th onClick={() => requestSort("account")} className="limits-table__th sortable-header">
                                                    Счет <SortIcon sortConfig={sortConfig} sortKey="account" />
                                                </th>
                                                <th onClick={() => requestSort("id")} className="limits-table__th sortable-header">
                                                    ID транзакции <SortIcon sortConfig={sortConfig} sortKey="id" />
                                                </th>
                                            </tr>
                                            </thead>
