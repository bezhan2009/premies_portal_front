...
const CatalogSelectModal = ({
  ...
  // Синхронизируем tempSelected при открытии или изменении selectedValues
  useEffect(() => {
    if (open) {
  ...
  const filteredOptions = React.useMemo(() => {
    ...
    return options.filter(
      (opt) =>
        ...
        (opt.value && String(opt.value).toLowerCase().includes(lowerQuery))
    );
  }, [options, searchQuery]);
  ...
  const handleSelectAllFiltered = () => {
    const filteredKeys = filteredOptions.map((opt) => opt.value);
    setTempSelected((prev) => [...new Set([...prev, ...filteredKeys])]);
  };
  ...
  const columns = [
    {
      ...
      key: "value",
      width: 150,
      sorter: (a, b) => String(a.value).localeCompare(String(b.value), "ru", { numeric: true }),
    },
    {
      title: "Наименование / Описание",
      dataIndex: "label",
      key: "label",
      sorter: (a, b) => String(a.label).localeCompare(String(b.label), "ru"),
    },
  ];
      ...
      footer={[
        ...
        >
          <Space>
            {multiple && (
              <>
                <Button onClick={handleSelectAllFiltered} size="middle">
                  Выбрать все найденные
                </Button>
                <Button onClick={handleClearSelection} size="middle" danger>
                  Очистить выбор
                </Button>
              </>
            )}
          ...
          <Space>
            ...
            <Button
              ...
              onClick={handleApply}
              size="middle"
              style={{ backgroundColor: "#eb2525", borderColor: "#eb2525" }}
            >
              Применить
            </Button>
          </Space>
    ...
    >
      ...
      >
        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>
          Найдено записей: <strong>{filteredOptions.length}</strong>
        </span>
        <Badge
          count={multiple ? `Выбрано: ${tempSelected.length}` : tempSelected.length ? `Выбрано` : `Не выбрано`}
          style={{ backgroundColor: "#eb2525" }}
        />
      </div>
      ...
      <AntTable
        ...
        rowSelection={{
          ...
          preserveSelectedRowKeys: true, // Сохранять выбор при переключении страниц
        }}
        scroll={{ y: 350 }}
      />
    </Modal>
...
export default function DashboardOperatorTransactionSearch() {
  ...
  const openCatalogModal = (field) => {
    ...
    switch (field) {
      case "atmId":
        title = "Справочник: Терминалы (ATM ID)";
        options = terminalOptions;
        selectedValues = atmId ? atmId.split(",") : [];
        multiple = true;
        break;
      case "excludeAtmIds":
        title = "Справочник: Исключить Терминалы (ATM ID)";
        options = terminalOptions;
        selectedValues = excludeAtmIds ? excludeAtmIds.split(",") : [];
        multiple = true;
        break;
      case "transactionTypes":
        title = "Справочник: Типы транзакций";
        options = transactionOptions;
        selectedValues = transactionTypes || [];
        multiple = true;
        break;
      case "excludeTransactionTypes":
        title = "Справочник: Исключить типы транзакций";
        options = transactionOptions;
        selectedValues = excludeTransactionTypes ? excludeTransactionTypes.split(",") : [];
        multiple = true;
        break;
      case "mcc":
        title = "Справочник: MCC коды";
        options = MCC_OPTIONS;
        selectedValues = mcc ? [String(mcc)] : [];
        multiple = false;
        break;
      case "excludeMcc":
        title = "Справочник: Исключить MCC коды";
        options = MCC_OPTIONS;
        selectedValues = excludeMcc ? excludeMcc.split(",") : [];
        multiple = true;
        break;
      case "responseCode":
        title = "Справочник: Response Code";
        options = RESPONSE_CODE_OPTIONS;
        selectedValues = responseCode ? [String(responseCode)] : [];
        multiple = false;
        break;
      default:
        return;
  ...
  const handleCatalogSelect = (selectedKeys) => {
    const { field } = catalogModal;
    switch (field) {
      case "atmId":
        setAtmId(selectedKeys.join(","));
        break;
      case "excludeAtmIds":
        setExcludeAtmIds(selectedKeys.join(","));
        break;
      case "transactionTypes":
        setTransactionTypes(selectedKeys);
        break;
      case "excludeTransactionTypes":
        setExcludeTransactionTypes(selectedKeys.join(","));
        break;
      case "mcc":
        setMcc(selectedKeys[0] || "");
        break;
      case "excludeMcc":
        setExcludeMcc(selectedKeys.join(","));
        break;
      case "responseCode":
        setResponseCode(selectedKeys[0] || "");
        break;
      default:
        break;
    }
  ...
  const transactionColumns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        ...getColumnSearchProps("cardNumber", "Номер карты"),
      },
      {
        title: "ID карты",
        dataIndex: "cardId",
        ...
        render: (_, transaction) => {
          ...
          return (
            <span className="amount-value">
              {formatAmount(transaction.amount, { ...transaction, transactionTypeNumber: transactionTypeValue })} {getCurrencyCode(transaction.currency)}
            </span>
          );
        },
      },
      ...
      },
      {
        ...
        width: 200,
        render: (value) => value || "N/A",
        ...getColumnSearchProps("account", "Счет"),
      },
      {
      ...
      <div className="block_info_prems content-page" align="center">
        <div className="processing-integration">
          ...
          <div className="txn-filter">
            {/* Идентификаторы */}
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <CreditCard className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">ИДЕНТИФИКАТОРЫ</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--3cols">
                  ...
                  <div className="txn-filter__field">
                    <div className="txn-filter__label-row">
                      <label htmlFor="atmId" className="txn-filter__label">
                        ATM ID
                      </label>
                      <button
                        type="button"
                        onClick={() => openCatalogModal('atmId')}
                        className="txn-filter__handbook-btn"
                        title="Справочник"
                      >
                        <BookOpen size={14} />
                      </button>
            ...
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <Hash className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">ПАРАМЕТРЫ ОПЕРАЦИИ</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--4cols">
                  <div className="txn-filter__field">
                  ...
                  <div className="txn-filter__field">
                    <div className="txn-filter__label-row">
                      ...
                      <button
                        type="button"
                        onClick={() => openCatalogModal('transactionTypes')}
                        className="txn-filter__handbook-btn"
                        title="Справочник"
                      >
                        <BookOpen size={14} />
                  ...
                  <div className="txn-filter__field">
                    ...
                    <Select
                      ...
                      value={mcc ? [mcc] : []}
                      onChange={(val) => setMcc(val[val.length - 1] || "")}
                      disabled={isLoading}
                      options={MCC_OPTIONS}
                      filterOption={(input, option) =>
                        (option?.label ?? '').toLowerCase().includes(input.toLowerCase())
                  ...
                  <div className="txn-filter__field">
                    <div className="txn-filter__label-row">
                      <label htmlFor="responseCode" className="txn-filter__label">
                        Response code
                      </label>
                      <button
                        type="button"
                        onClick={() => openCatalogModal('responseCode')}
                        className="txn-filter__handbook-btn"
                        title="Справочник"
                      >
                        <BookOpen size={14} />
            ...
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <Coins className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">СУММЫ</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--5cols">
                  <div className="txn-filter__field">
            ...
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <Link className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">ВАЛЮТЫ И ПРОЧЕЕ</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--4cols">
                  <div className="txn-filter__field">
                  ...
                  <div className="txn-filter__field">
                    <label htmlFor="conCurrency" className="txn-filter__label">
                      Валюта конверс.
                    </label>
                    <Select
                      showSearch
            ...
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <Smartphone className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">ОПЛАТА ТЕЛЕФОНОМ</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--1col">
                  <div className="txn-filter__field">
            ...
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <Ban className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">ИСКЛЮЧЕНИЯ</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--4cols">
                  <div className="txn-filter__field">
                  ...
                  <div className="txn-filter__field">
                    <label htmlFor="excludeAccounts" className="txn-filter__label">
                      Искл. счета
                    </label>
                    <input
                      type="text"
            ...
            <div className="txn-filter__group">
              <div className="txn-filter__group-header">
                <Calendar className="txn-filter__group-icon" />
                <span className="txn-filter__group-title">ПЕРИОД</span>
              </div>
              <div className="txn-filter__group-card">
                <div className="txn-filter__fields txn-filter__fields--4cols">
                  <div className="txn-filter__field">
            ...
            {/* Действия */}
            <div className="txn-filter__actions-wrapper">
              <div className="txn-filter__actions">
                ...
                >
                  {!isLoading && <Search size={16} className="btn-icon" />}
                              ...
                              <td className="limits-table__td limits-table__td--value">
                                ...
                                >
                                  {(() => {
                                    ...
                                    return formatAmount(
                                      amountTJS,
                                      { ...transaction, transactionTypeNumber: transactionTypeValue },
                                    );
                                  })()}
                                </span>
                              </td>
