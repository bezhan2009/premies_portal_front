...
export default function DashboardOperatorProcessingTransactions() {
    ...
    const handleSearchTypeChange = (e) => {
        // Если у пользователя ограниченный доступ, запрещаем смену типа поиска
        if (isLimitedAccess) {
            showAlert("У вас ограниченный доступ. Можно просматривать только историю одной карты", "warning");
            return;
        }
    ...
    // Показываем предупреждение об ограниченном доступе
    useEffect(() => {
        if (isLimitedAccess) {
                    ...
                    <div className="processing-integration__container">
                        <div className="processing-integration__header">
                            <h1 className="processing-integration__title">
                                ...
                                {isLimitedAccess && <span style={{ color: '#ffa500', fontSize: '0.8em', marginLeft: '10px' }}>(Ограниченный доступ)</span>}
                            </h1>
                            <p className="processing-integration__subtitle">
                                {isLimitedAccess ? "Просмотр истории одной карты" : "Поиск транзакций без ограничений"}
                            </p>
                        </div>
