...
const TableReportsKc = ({ month, year }) => {
    ...
    try {
      ...
      if (existingQuality?.ID) {
        const response = await fetch(`${backendURL}/service-quality/${existingQuality.ID}`, {
          ...
          body: JSON.stringify({ call_center: call_center_value }),
        });
        if (!response.ok) throw new Error("Ошибка при PATCH");
        setData((prev) => prev.map((item) => item.ID === row.ID ? { ...item, ServiceQuality: [{ ...item.ServiceQuality?.[0], call_center: call_center_value }] } : item));
      } else {
        const createdAt = new Date(Date.UTC(year, month - 1, 1)).toISOString();
        const response = await fetch(`${backendURL}/service-quality`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
          body: JSON.stringify({ call_center: call_center_value, coefficient: 0, complaint: 0, tests: 0, WorkerID: row.ID, CreatedAt: createdAt, UpdatedAt: createdAt }),
        });
        if (!response.ok) throw new Error("Ошибка при POST");
        const created = await response.json();
        setData((prev) => prev.map((item) => item.ID === row.ID ? { ...item, ServiceQuality: [{ ID: created.ID, call_center: call_center_value }] } : item));
      }
  ...
  const handleExport = () => {
    const columns = [
      ...
      { key: (row) => row.ServiceQuality?.[0]?.call_center ?? "", label: "Средняя оценка" },
    ];
      ...
      >
        ...
        <Table.Column
          ...
          render={(_, row) => {
            ...
            const score = row.ServiceQuality?.[0]?.call_center ?? "";
            if (isEditing) {
              return (
                <input
                  type="number"
