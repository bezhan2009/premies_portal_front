...
export default function InternetBankingDocxSettings({template,onChange}) {
 ...
 return <section className="docx-editor-card">
  ...
  <label><input style={{width:18,height:18,padding:0,flexShrink:0,display:"inline-block",verticalAlign:"middle",marginRight:6}} type="checkbox" checked={config.enabled} onChange={e=>update({enabled:e.target.checked})}/> Показывать клиентам интернет-банка</label>
  ...
  <div className="docx-form-grid" style={{marginTop:16}}>
   ...
   <div className="docx-field"><span>Доступные форматы</span>{["pdf","docx"].map(format=><label key={format} style={{display:"flex",alignItems:"center",gap:6,fontWeight:400}}><input style={{width:18,height:18,padding:0,flexShrink:0,display:"inline-block",verticalAlign:"middle",marginRight:6}} type="checkbox" checked={config.formats.includes(format)} onChange={e=>{const formats=e.target.checked?[...config.formats,format]:config.formats.filter(item=>item!==format);if(formats.length)update({formats,defaultFormat:formats.includes(config.defaultFormat)?config.defaultFormat:formats[0]});}}/> {format.toUpperCase()}</label>)}</div>
   ...
   {config.placement==="account"&&<label><input style={{width:18,height:18,padding:0,flexShrink:0,display:"inline-block",verticalAlign:"middle",marginRight:6}} type="checkbox" checked={config.requiresPeriod} onChange={e=>update({requiresPeriod:e.target.checked})}/> Запрашивать период выписки (до 31 дня)</label>}
  </div>
