  ...
  if (active && payload && payload.length) {
    return (
      ...
      >
        ...
        <div>
          <span style={{ color: "#417cd5" }}>Всего карт:</span>{" "}
          {formatNumber(payload[0]?.value || 0)}
        </div>
        <div>
          <span style={{ color: "#41b8d5" }}>Выдано в этом месяце:</span>{" "}
          {formatNumber(payload[1]?.value)}
        </div>
        <div>
          <span style={{ color: "#6ce5e8" }}>
            Активные карты за этот месяц:
          </span>{" "}
          {formatNumber(payload[2]?.value)}
        </div>
      </div>
    );
  }
