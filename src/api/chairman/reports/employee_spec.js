...
export const fetchEmployee = async (month, employeeURL) => {
  ...
  if (parts[0] === "*" && parts[parts.length - 1] === "stats") {
    ...
    }/workers/card-details/stats?month=${monthParam}&year=${yearParam}`;
    ...
    const transformed = {
      ...
      CardTurnovers: [
        {
          activated_cards:
            cards_for_month === 0 ? 0 : stats.activated_cards || 0,
          card_turnovers_prem: 0,
          WorkerID: 0,
        },
      ],
      CardSales: [
        {
          ...
          cards_sailed_in_general:
            cards_for_month === 0 ? 0 : stats.cards_in_general || 0,
          cards_prem: 0,
          WorkerID: 0,
        },
      ],
...
export const getAllOffices = async () => {
  try {
    const res = await apiClient("/office");
    return res.data;
  } catch (e) {
    console.log(e);
    return [];
