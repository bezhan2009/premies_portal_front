...
export function saveInternetBankingClient(payload, options = {}) {
  ...
  const body = {
    ...
    people: (payload.people || []).map((person) => ({
      ...
      allowed_ips: person.allowed_ips || [],
...
export function listInternetBankingTranslations(options = {}) {
  return requestInternetBanking(`${operatorPath}/translations`, { headers: authHeaders() }, options);
}

export function saveInternetBankingTranslations(items, options = {}) {
  return requestInternetBanking(`${operatorPath}/translations`, {
    method: "PUT",
    headers: authHeaders(true),
