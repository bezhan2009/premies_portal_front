  ...
  const applyFilters = (data) => {
    return data.filter((row) => {
      ...
      return (
          ...
          (!filters.resident || (filters.resident === "Да" ? row.is_resident : !row.is_resident)) &&
          (!filters.card || row.card_name.toLowerCase().includes(filters.card.toLowerCase()))
      );
    });
  };

  const headers = ["Телефон", "Кодовое слово", "Имя на карте", "Пол", "Резидент", "Документ", "ИНН", "Адрес", "Карта"];
          ...
          <main>
            ...
            {showFilters && (
                <div className="filters animate-slideIn">
                  <input placeholder="ФИО" onChange={(e) => handleFilterChange("fullName", e.target.value)} />
                  <input placeholder="Телефон" onChange={(e) => handleFilterChange("phone", e.target.value)} />
                  <select onChange={(e) => handleFilterChange("resident", e.target.value)}>
                    <option value="">Резидент</option>
                    <option value="Да">Да</option>
                    <option value="Нет">Нет</option>
                  </select>
                  <input placeholder="Карта" onChange={(e) => handleFilterChange("card", e.target.value)} />
                </div>
            )}
