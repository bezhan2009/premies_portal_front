...
export default function PVNTransactionsList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      showAlert(
        err.message || "Ошибка загрузки данных. Проверьте сервер.",
        "error",
      );
      setTableData([]);
    } finally {
    ...
    data?.pvn_from_datetime,
    data?.pvn_to_datetime,
    backendABS,
    token,
    defaultFromDateTime,
    defaultToDateTime,
  ...
  const handleExport = () => {
    ...
    const rows = selectedData.map((row) => {
      ...
      for (const header of allHeaders) {
        ...
        if (header.startsWith("payed_")) {
          ...
          obj[label] =
            typeof row[header] === "number" ? row[header] / 100 : row[header];
        } else if (header === "currency") {
          obj[label] = getCurrencySymbol(row[header]);
        } else if (
          header.includes("Date") ||
          header === "localTransactionDate"
        ) {
    ...
    XLSX.writeFile(
      workbook,
      `pvn_transactions_${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  ...
  const selectAllUnpaid = () => {
    setSelectedRows(
      sortedData.filter((row) => !isPaidTransaction(row)).map((r) => r.id),
    );
    setSelectAll(false);
  };
  ...
  // Определяем валюту для отображения в сумме (берём из первой строки, если есть)
  const selectedCurrencySymbol = useMemo(() => {
    if (sortedData.length === 0) return "";
    return getCurrencySymbol(sortedData[0]?.currency);
  }, [sortedData]);
                    ...
                    {sortedData.map((row) => {
                      ...
                      return (
                        <tr
                          key={row.id}
                          style={{
                            backgroundColor: paid ? "#e6ffe6" : "transparent",
                          }}
                        >
                          <td>
                            <input
                          ...
                          <td className="active-table">
                            <button
                              ...
                              style={{
                                ...
                                border: "none",
                                cursor:
                                  paid || isPaying ? "not-allowed" : "pointer",
