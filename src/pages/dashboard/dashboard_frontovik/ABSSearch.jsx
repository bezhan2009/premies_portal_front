...
const TYPE_SEARCH_CLIENT = [
    { label: "Поиск по Номеру телефона", value: "phone", inputLabel: "Номер телефона" },
    {
        label: "Поиск по Номеру индекса",
        value: "client-index",
        inputLabel: "Номер индекса",
    },
    { label: "Поиск по INN", value: "inn", inputLabel: "INN" },
];
...
const normalizeClientData = (client, searchType) => {
    ...
    // Если это поиск по коду клиента или ИНН (новый формат)
    if (searchType === "client-index" || searchType === "inn" || client.ColvirReferenceId) {
        return {
            phone: client.ContactData?.[0]?.Value || "",
            arc_flag: client.AddInfoList?.find(item => item.Code === "ARC_FLAG")?.Value || "",
            client_type_name: client.TypeExt?.Name || client.Type || "",
            ban_acc_open_flag: "",
            ...
            ltn_surname: client.LatinLastName || client.ltn_surname || "",
            ltn_name: client.LatinFirstName || client.ltn_name || "",
            ltn_patronymic: client.LatinMiddleName || client.ltn_patronymic || "",
            tax_code: client.TaxIdentificationNumber?.Code || client.tax_code || "",
            identdoc_name: client.IdentDocs?.[0]?.DocType?.Name || "",
            ...
            identdoc_orgname: client.IdentDocs?.[0]?.IssueOrg || "",
            sv_id: client.ExternalSystemCodes?.ExternalCode?.[0]?.Code || "",
            ...
            Dreg: client.Dreg || "",
            AddressString: client.DetailedAddresses?.[0]?.AddressString || "",
        };
    }

    // Старый формат (поиск по телефону)
    return {
        phone: client.phone || "",
        arc_flag: client.arc_flag || "",
        client_type_name: client.client_type_name || "",
        ban_acc_open_flag: client.ban_acc_open_flag || "",
        dep_code: client.dep_code || "",
        client_code: client.client_code || "",
        surname: client.surname || "",
        name: client.name || "",
        patronymic: client.patronymic || "",
        ltn_surname: client.ltn_surname || "",
        ltn_name: client.ltn_name || "",
        ltn_patronymic: client.ltn_patronymic || "",
        tax_code: client.tax_code || "",
        identdoc_name: client.identdoc_name || "",
        identdoc_series: client.identdoc_series || "",
        identdoc_num: client.identdoc_num || "",
        identdoc_date: client.identdoc_date || "",
        identdoc_orgname: client.identdoc_orgname || "",
        sv_id: client.sv_id || "",
    };
};
...
export default function ABSClientSearch() {
    ...
    const formatPhoneNumber = (value) => {
        ...
        if (match) {
            return `${match[1]}${match[2] ? ` (${match[2]}) ` : ""}${match[3]}${match[4] ? `-${match[4]}` : ""}${match[5] ? `-${match[5]}` : ""}`;
        }
        return value;
    };
    ...
    const handleSearchChange = (e) => {
        ...
        // Форматируем только для телефона
        if (selectTypeSearchClient === "phone") {
    ...
    const getSearchEndpoint = () => {
        const currentType = TYPE_SEARCH_CLIENT.find(t => t.value === selectTypeSearchClient);
        switch (currentType.value) {
            case "phone":
                return `/client/info?phoneNumber=`;
            case "client-index":
                return `/client/client-index?clientIndex=`;
            case "inn":
    ...
    const handleSearchClient = async () => {
        if (!searchValue) {
            showAlert("Пожалуйста, введите значение для поиска", "error");
            return;
        }

        let formattedSearch = searchValue.trim();

        // Для телефона оставляем только цифры
        if (selectTypeSearchClient === "phone") {
            formattedSearch = formattedSearch.replace(/\D/g, "");
        }

        // Для ИНН тоже только цифры
        if (selectTypeSearchClient === "inn") {
            formattedSearch = formattedSearch.replace(/\D/g, "");
        }

        // Для client-index можно оставить как есть (может содержать точки)

        try {
            let isMobile = null;

            if (selectTypeSearchClient === "phone") {
                isMobile = await getUserInfoPhone(formattedSearch);
            }
            ...
            const url = `${API_BASE_URL}${endpoint}${encodeURIComponent(formattedSearch)}`;
            ...
            if (Array.isArray(data)) {
                if (selectTypeSearchClient === "phone") {
                    // Для телефона - старый формат
                    normalizedData = data;
                } else {
                    // Для ИНН - новый формат
                    normalizedData = data.map(client => normalizeClientData(client, selectTypeSearchClient));
                }
            } else {
                // Для поиска по client-index возвращается один объект
    ...
    const tableData = selectedClient
        ? [
            ...
            { label: "Полное имя", key: "LongName", value: selectedClient.LongName },
            { label: "Дата рождения", key: "BirthDate", value: selectedClient.BirthDate },
            { label: "Статус", key: "State", value: selectedClient.State },
            { label: "Пол", key: "Sex", value: selectedClient.Sex },
            { label: "Дата открытия", key: "FromDate", value: selectedClient.FromDate },
            { label: "Дата регистрации", key: "Dreg", value: selectedClient.Dreg },
            { label: "Адрес", key: "AddressString", value: selectedClient.AddressString },
        ]
        : [];
    ...
    const handleSelectTypeChange = (e) => {
        ...
        setSelectTypeSearchClient(newType);
        // Очищаем поле ввода при смене типа поиска
        setSearchValue("");
        setDisplayValue("");
    };
                                        ...
                                        >
                                            {clientsData.map((client, index) => (
                                                <option key={index} value={index}>
                                                    ...
                                                    {client.client_code && ` (Код: ${client.client_code})`}
                                                </option>
                                            ))}
                                        </select>
