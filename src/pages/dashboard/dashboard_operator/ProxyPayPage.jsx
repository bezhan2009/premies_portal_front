...
import {forecastText,ledgerCanPay,ledgerDate,ledgerDirection,ledgerMoney,ledgerStatus} from './proxyLedgerUtils.js';
...
const states=['manual','sending','accepted','executed','unknown','error'].map(value=>({value,label:ledgerStatus(value)}));
const statusTag=value=><Tag color={{executed:'green',error:'red',unknown:'orange',sending:'blue',accepted:'gold'}[value]}>{ledgerStatus(value)}</Tag>;
...
function TransferLedger(){
 ...
 const [selected,setSelected]=useState(null),[prepared,setPrepared]=useState(null);
 ...
 const pending=data.items.some(r=>['sending','accepted'].includes(r.payment_status)||['sending','accepted'].includes(r.conversion_status));
 ...
 const openStage=async(row,kind)=>{
  const current=++generation.current;setSelected({row,kind});setPrepared(null);setModalError('');
  if(kind==='conversion'&&!row.quote_at)return;
  setBusy('prepare');try{const value=await loadPrepared(row,kind);if(current===generation.current){setPrepared(value);setSelected({row:{...row,...value.transfer},kind});}}catch(e){if(current===generation.current)setModalError(errorText(e));}finally{if(current===generation.current)setBusy('');}
 };
 ...
 const pay=()=>Modal.confirm({title:'Подтвердите проводку',content:<Space direction="vertical"><strong>{selected.kind==='payment'?'Платежка':'Конвертация'} · №{selected.row.utrnno}</strong><span>{prepared?.input.amount} {selected.kind==='payment'?'TJS':'RUB'}</span><span>Списание: {prepared?.input.payer_iban}</span><span>Зачисление: {prepared?.input.beneficiary_iban}</span><span>Запрос будет отправлен в АБС один раз.</span></Space>,okText:'Оплатить',cancelText:'Отмена',onOk:async()=>{
 ...
 const checkStatuses=async(row)=>{setBusy(`status-${row.id}`);try{for(const id of [row.payment_operation_id,row.conversion_operation_id].filter(Boolean))await apiClient.post(`${URL}/operations/${id}/status`,{},{timeout:145000});refresh();}catch(e){setError(errorText(e));}finally{setBusy('');}};
 const columns=[
  ...
  {title:'Зачисление, TJS',dataIndex:'amount_minor',align:'right',width:140,render:ledgerMoney},
  {title:'RUB на момент операции',dataIndex:'rub_minor',align:'right',width:165,render:ledgerMoney},
  {title:'TJS по конвертации',dataIndex:'tjs_minor',align:'right',width:155,render:ledgerMoney},
  ...
  {title:'Платежка / конвертация',width:235,render:(_,r)=><Space direction="vertical" size={6}><Tooltip title={r.payment_message}>{statusTag(r.payment_status)}</Tooltip><Tooltip title={r.conversion_message}>{statusTag(r.conversion_status)}</Tooltip>{r.block_reason&&<Typography.Text type="danger">{r.block_reason}</Typography.Text>}</Space>},
  {title:'Действия',key:'actions',width:190,fixed:'right',render:(_,r)=><Space direction="vertical" size={4}><Button size="small" onClick={()=>openStage(r,'payment')}>Платежка</Button><Button size="small" onClick={()=>openStage(r,'conversion')}>Конвертация</Button>{(r.payment_operation_id||r.conversion_operation_id)&&<Button type="link" size="small" loading={busy===`status-${r.id}`} onClick={()=>checkStatuses(r)}>Проверить статусы</Button>}</Space>},
 ];
 return <div className="proxy-ledger">
  ...
  <Row gutter={[16,16]} className="proxy-metrics">
   <Col xs={24} lg={8}><Card><Statistic title="Баланс Proxy Pay · RUB" value={metrics?.balance??'—'} precision={2}/><div className="proxy-metric-note">{forecastText(metrics)}</div><Typography.Text type="secondary">За 7 дней: суммы RUB известны для {metrics?.week_quoted??0} из {metrics?.week_count??0} операций</Typography.Text></Card></Col>
  ...
  <Alert showIcon type="info" message="Автоматически загружаются только операции. Проводки выполняются вручную." description="Исторические операции защищены от повторных проводок. Суммы RUB и TJS появляются после получения курсов."/>
  ...
  <Table rowKey="id" dataSource={data.items} columns={columns} loading={loading} scroll={{x:1580}} pagination={{current:page,pageSize:50,total:data.total,showSizeChanger:false,onChange:setPage,showTotal:total=>`${total} операций`}}/>
  <Modal open={!!selected} title={`${selected?.kind==='payment'?'Платежка':'Конвертация'} · №${selected?.row.utrnno??''}`} width={860} maskClosable={false} onCancel={()=>{if(!busy){generation.current++;setSelected(null);setPrepared(null);}}} closable={!busy} footer={selected&&<Space><Button disabled={!!busy} onClick={()=>setSelected(null)}>Закрыть</Button>{selected.kind==='conversion'&&!selected.row.conversion_operation_id&&<Button loading={busy==='quote'} disabled={!!busy&&busy!=='quote'} onClick={quote}>Получить курсы</Button>}<Button type="primary" loading={busy==='pay'} disabled={!!busy||!prepared||!ledgerCanPay(selected.row,selected.kind)} onClick={pay}>Оплатить</Button></Space>}>
   {selected&&<Space direction="vertical" size={16} style={{width:'100%'}}>
    {selected.row.historical&&<Alert type="info" message="Историческая операция: повторная проводка запрещена"/>}
    ...
    <Descriptions bordered size="small" column={2} items={[
     ...
     {key:'amount',label:'Зачисление',children:`${ledgerMoney(selected.row.amount_minor)} TJS`},{key:'rub',label:'RUB на момент операции',children:ledgerMoney(selected.row.rub_minor)},
     {key:'tjs',label:'TJS по конвертации',children:ledgerMoney(selected.row.tjs_minor)},{key:'quote',label:'Расчёт',children:ledgerDate(selected.row.quote_at)},
    ]}/>
    ...
    ]}/><details><summary>Запрос в АБС</summary><pre className="proxy-xml">{prepared.request_xml}</pre></details></>}
   </Space>}
  </Modal>
 </div>;
}
