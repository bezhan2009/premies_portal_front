...
import { Cpu, Edit3, Landmark, Plus, RefreshCw, Search, ShieldCheck, Trash2, Users } from "lucide-react";
...
export default function InternetBankingPage() {
  ...
  const clientColumns = [
    ...
    { title: "Клиент", dataIndex: "display_name", render: (value) => value || "—" },
    {
      title: "Пользователи",
      dataIndex: "people",
      render: (people = []) => <Space wrap>{people.map((person) => <Tag key={person.access_id || person.id}>{person.full_name}</Tag>)}</Space>,
    },
    {
      title: "Телефоны",
      dataIndex: "people",
          ...
          {(fields, { add, remove }) => <>
            ...
            {fields.map((field, index) => <Card className="ib-person-card" key={field.key} title={`Пользователь ${index + 1}`} extra={fields.length > 1 && <Button type="text" danger icon={<Trash2 size={16} />} onClick={() => remove(field.name)}>Удалить</Button>} bordered>
              ...
              <Row gutter={16}>
                ...
                <Col xs={24} lg={4}><Form.Item name={[field.name, "isActive"]} label="Активен" valuePropName="checked"><Switch /></Form.Item></Col>
              </Row>
              <Row gutter={16}>
