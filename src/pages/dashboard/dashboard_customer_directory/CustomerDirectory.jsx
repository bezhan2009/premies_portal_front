...
export default function CustomerDirectory() {
  ...
  const applySearch = (event) => {
    ...
    setPage(1);
    const next={...filters,search:draftSearch.trim()};setFilters(next);setAppliedFilters(next);
  };
  ...
  return (
    <main className="customer-directory content-page">
      ...
      <section className="customer-directory__toolbar" data-collapsed={!filtersOpen}>
        ...
        <div className="customer-choice customer-employee-choice" data-active={Boolean(filters.creator)}><label htmlFor="customer-creator">Оформил</label><Select id="customer-creator" aria-label="Оформил" showSearch allowClear optionFilterProp="label" value={filters.creator || undefined} placeholder="Все сотрудники" onChange={value=>updateFilter('creator',value||'')} options={(filterOptions.creators||[]).map(item=>({value:item.value,label:`${item.label} (${item.value})`}))} style={{width:'100%'}} /></div>
        ...
        <div className="customer-filter-footer">
          <button type="button" className="customer-reset-button" onClick={() => { setFilters(INITIAL_FILTERS); setAppliedFilters(INITIAL_FILTERS);setDraftSearch(""); setPage(1); }}><X size={17}/>Сбросить фильтры</button>
      ...
      <section className="customer-table-wrap" aria-busy={loading}>
