...
$bashLines.Add('  if ! $compose_command -f docker-compose.yml -f .deploy-internet-banking-token.override.yml up -d --no-deps go-backend abs_service; then')
...
$bashLines.Add('  if ! $compose_command -p internet_banking -f internet_banking_backend/deploy/docker-compose.yml up --build -d; then')
